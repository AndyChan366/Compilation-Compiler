...
int get_size(tree init_tree) {
	...
	else if (init_tree->NodeOpType == ArrayTypeOp) {
		int returnVal;
        returnVal=0;
...
void array_init(char* Lable, tree init_tree, int is_beyond_1d) {
	if ((init_tree->NodeOpType == ArrayTypeOp) && !is_beyond_1d) {
	...
	if (!(init_tree->NodeOpType == ArrayTypeOp) ){
        if ((NodeKind(RightChild(init_tree)) == EXPRNode) && ((RightChild(init_tree))->NodeOpType == ArrayTypeOp)) {
			array_init(Lable, RightChild(init_tree), 1);
		}
		else if ((NodeKind(init_tree) == EXPRNode) && (init_tree->NodeOpType == BoundOp)) {
...
void emit_if(tree syntax_tree, int EndIfFreq) {
	...
	if( !(TreeTemp->NodeOpType == CommaOp)) {
...
void emit_data(tree TreeDecl, int* offset_ptr) {
	...
	char* ParNAme = String_table + GetAttr(IntVal(RightChild(cur_class)), NAME_ATTR);
	char* var_name;
	if (LeftChild(FuncNAme) == NullExp()){
		var_name = String_table + GetAttr(IntVal((FuncNAme)), NAME_ATTR);
	}
	else {
		var_name = String_table + GetAttr(IntVal(LeftChild(FuncNAme)), NAME_ATTR);
	}
	...
	*offset_ptr += SizeObj;
	...
	if (NodeKind(RightChild(TreeType)) == EXPRNode) {
		if ((RightChild(TreeType))->NodeOpType == ArrayTypeOp) {
...
void emit_call(tree func_tree, tree cur_class) {
	...
	if (RightChild(FuncNAme) == NullExp()) {
		ParNAme = String_table + GetAttr(IntVal(RightChild(cur_class)), NAME_ATTR);
		function_name = String_table + GetAttr(IntVal(LeftChild(FuncNAme)), NAME_ATTR);
	}
	else {
		...
		ParNAme = String_table + GetAttr(IntVal(class_tree), NAME_ATTR);
		function_name = String_table + GetAttr(IntVal(LeftChild(LeftChild(RightChild(FuncNAme)))), NAME_ATTR);
	}
	...
	for (i = 0; (i <= 3) && (param_tree != NullExp()); i++) {
		...
		if (NodeKind(cur_param) == STRINGNode) {
			char* properly_quoted_str = malloc(strlen(String_table + IntVal(cur_param)));
			// quote_it_right(properly_quoted_str, String_table + IntVal(cur_param));
			fprintf(output_file, ".data\n");
			fprintf(output_file, ".align 4\n");
			// fprintf(output_file, "string_%d: .asciiz %s\n", str_num, properly_quoted_str);
			fprintf(output_file, "string_%d: .asciiz \"%s\"\n", str_num, String_table + IntVal(cur_param));
			free(properly_quoted_str);
			fprintf(output_file, ".text\n");
 			fprintf(output_file, "la $a%d string_%d      # string param\n", i, str_num++);
		}
		else if (NodeKind(cur_param) == EXPRNode) {
			if (cur_param->NodeOpType == VarOp) {
...
int handle_function_params(tree syntax_tree, int cur_offset, int cur_param_num) {
	...
	if (syntax_tree->NodeOpType == RArgTypeOp) {
...
void create_function(tree syntax_tree, tree cur_class) {
	if (!strcmp(String_table + GetAttr(IntVal(LeftChild(LeftChild(syntax_tree))), NAME_ATTR), "main")) {
		fprintf(output_file, "la $gp base\n");
		fprintf(output_file, "main:\n");
	}
	else {
		fprintf(output_file, "%s_%s:\n", String_table + GetAttr(IntVal(RightChild(cur_class)), NAME_ATTR), String_table + GetAttr(IntVal(LeftChild(LeftChild(syntax_tree))), NAME_ATTR));
	}
...
int handle_function_decls(tree syntax_tree, int cur_offset) {
	...
	while (TreeDecl != NullExp()) {
		...
		if (!(init_tree == NullExp())){
			if (NodeKind(init_tree) == EXPRNode && (init_tree->NodeOpType == ArrayTypeOp)) { 
				tree init_value_tree = LeftChild(init_tree);
				if (init_value_tree->NodeOpType == CommaOp) {	 
...
void evaluate_addr(tree syntax_tree) {
	...
	// tree nearest_class_copy = get_class(LeftChild(syntax_tree));
	while (TreeTemp != NullExp()) {
		if ((LeftChild(TreeTemp))->NodeOpType == IndexOp) {
		...
		else if ((LeftChild(TreeTemp))->NodeOpType == FieldOp) { 
...
tree class_var_access(tree syntax_tree, tree base_class) {
	char* ParNAme ;
    tree child_tree;
    ParNAme = String_table + (GetAttr(IntVal(base_class) + 3, NAME_ATTR));
	// printf("%d\n",IntVal(base_class));
	// printf("%s\n",ParNAme);
	char* var_name;
	if (LeftChild(syntax_tree) != NullExp()) {
		child_tree = LeftChild(syntax_tree);
		var_name = String_table + GetAttr(IntVal(child_tree), NAME_ATTR);
	}
	else {
	...
	if (syntax_tree == NullExp()) 
		...
		switch (syntax_tree->NodeOpType) {
			case ClassDefOp:
				...
				while (TreeDecl != NullExp() && ((RightChild(TreeDecl))->NodeOpType == MethodOp)) {
					TreeDecl = LeftChild(TreeDecl);
				}
