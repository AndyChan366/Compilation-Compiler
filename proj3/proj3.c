...
{
    printf("Semantic Error--line: %d, ", yyline-1);
    switch (type)
    {
        case STACK_OVERFLOW:
        ...
        case TYPE_MIS:
            ...
            break;
        case RE_MAIN:
            printf("symbol main: redeclared.\n");
            break;
        case OPER_MIS:
            printf("this operator doesn't match the operand.\n");
            break;
        case ARGU_MIS:
            printf("routine %s:argument type of the %s parameter different with the previous declaration.\n",getname(id),seq_str(seq));
        case RETN_MIS:
            printf("symbol %s: return value doesn't match the type of the method return.\n", getname(id));
            break;
        default:
            printf("error type: %d.\n", type);
    }
